...
using Epoxide.Linq.Expressions.Fingerprints;

namespace Epoxide.Linq.Expressions;

/// <summary>
/// Implements caching around LambdaExpression.Compile() so that equivalent
/// expression trees only have to be compiled once.
/// </summary>
/// <remarks>
/// The caching expression tree compiler was copied from MVC core to MVC Futures
/// so that Futures code could benefit from it and so that it could be exposed as
/// a public API. This is the only public entry point into the system.
/// </remarks>
public static class CachedExpressionCompiler
{
    ...
    public static object? Evaluate(Expression arg)
    {
        ...
        return Compiler<object?, object?>.Compile(lambdaExpr)(null);
    }
